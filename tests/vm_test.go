			...
			Context("Using RunStrategyManual", func() {
				...
				DescribeTable("with a failing VMI and the kubevirt.io/keep-launcher-alive-after-failure annotation", func(keepLauncher string) {
					...
					Eventually(func() bool {
						...
						Expect(err).ToNot(HaveOccurred())
						_, err = libpod.GetPodByVirtualMachineInstance(vmi, vm.Namespace)
						if errors.IsNotFound(err) {
							return false
						}
						Expect(err).ToNot(HaveOccurred())
						return true
					}).WithTimeout(60*time.Second).WithPolling(5*time.Second).Should(BeTrue(), "virt-launcher should be created eventually")

					getComputeContainerStateRunning := func() *k8sv1.ContainerStateRunning {
					...
					if toKeep, _ := strconv.ParseBool(keepLauncher); toKeep {
						Consistently(getComputeContainerStateRunning).WithTimeout(10 * time.Second).WithPolling(1 * time.Second).ShouldNot(BeNil())
					} else {
						Eventually(getComputeContainerStateRunning).WithTimeout(100 * time.Second).WithPolling(1 * time.Second).Should(BeNil())
					}
				},
